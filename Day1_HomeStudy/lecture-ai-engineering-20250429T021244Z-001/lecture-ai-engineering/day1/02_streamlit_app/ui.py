...
# --- チャットページのUI ---
def display_chat_page(tokenizer, model):
    """チャットページのUIを表示する"""
    st.subheader("あなたの想いを入力してね")
    user_question = st.text_area("質問", key="question_input", height=100)

    if st.button("🌟 おしゃべりスタート！"):
        with st.spinner("りんなちゃんが考え中..."):
            answer, response_time = generate_response(tokenizer, model, user_question)
            st.success("生成されました！")

            st.markdown("### りんなちゃんの考え")
            st.markdown(answer)

            st.markdown(f"**応答時間：{response_time:.2f}秒**")
